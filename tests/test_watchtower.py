...
class WatchtowerUnitTests(unittest.TestCase):
    ...
    def test_status_page_uses_incident_first_dashboard_layout(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("emaPeriod: 20", html)
            self.assertIn("emaPeriod: 50", html)
            self.assertIn("emaPeriod: 100", html)
            self.assertIn("emaPeriod: 200", html)
            self.assertIn("emaPeriod: 365", html)
